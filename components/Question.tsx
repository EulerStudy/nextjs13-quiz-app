...
const Question = ({question, withId, checkEnabled, getAnswer}: Props) => {
  ...
  return (
    <div
      ...
      </div>
      {
        question.options.map((option)=>(
          <label 
            ...
            bg-white shadow-sm rounded-lg my-2 px-4 py-2 border-2 duration-200
            border-transparent hover:bhrder-blue-600 cursor-pointer'>
            {
              isAnswered &&
              ( isTrue 
                  ? option.id === question.answer 
                      ? (<CheckCircleIcon className='w-6 h-6 stroke-green-600'></CheckCircleIcon>)
                      : (<></>)
                  : option.id === question.answer 
                      ? (<CheckCircleIcon className='w-6 h-6 stroke-green-600'></CheckCircleIcon>)
                      : <XCircleIcon className='w-6 h-6 stroke-red-600'></XCircleIcon>
              )
            }
            <input
      ...
      {
        checkEnabled && transitions((styles, item) => 
          answer > 0 && !item &&(
            <animated.div style={{overflow:'hidden', ...styles}}>
              <div
                ...
                className='bg-blue-600 p-2 text-slate-50 rounded text-center shadow
                hover:bg-blue-500 duration-200 cursor-pointer'
              >
                답변을 확인하세요.
              </div>
            </animated.div>
